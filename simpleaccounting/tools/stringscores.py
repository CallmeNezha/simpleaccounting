...
def str2regex(query, ignore_case=True):
...
def matchingScore(query, choice, ignore_case=True, apply_regex=True,
                  template='{}'):
    ...
    if apply_regex:
        pattern = str2regex(query, ignore_case=ignore_case)
        ...
        if any(exact_words) or any(partial_words):
            ...
            enriched_text = htmlEscape(original_choice[:pos_start]) +\
                template.format(htmlEscape(original_choice[pos_start:pos_end])) +\
                htmlEscape(original_choice[pos_end:])
        ...
        if any(exact_words):
            ...
            for char in query:
                if char != u'' and char in temp_text:
                    ...
                    enriched_text[index] = template.format(htmlEscape(text[index]))
                    text[index] = sep
...
def findMatchingChoices(query, choices, ignore_case=True, template='{}',
    ...
    query = query.replace(' ', '')
    pattern = str2regex(query, ignore_case)
    results = []

    for choice in choices:
        r = re.search(pattern, choice)
        if query and r:
            result = matchingScore(query, choice, ignore_case=ignore_case,
...
def test():
    ...
    names = ['close pane', 'debug continue', 'debug exit', 'debug step into',
             ...
             'switch to ipythonconsole', 'switch to onlinehelp',
    ...
    a = findMatchingChoices('lay', names, template=template, )
    b = findMatchingChoices('lay', names, template=template, valid_only=True,
                            sort=True)
    # Full results
    for r in a:
        print(r)
