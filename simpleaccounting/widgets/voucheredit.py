...
class CurrencyItemDelegate(QtWidgets.QStyledItemDelegate):
    def createEditor(self, parent, option, index):
        # Create a QComboBox as editor
        combobox = QtWidgets.QComboBox(parent)
        if current_account := index.model().data(index.model().createIndex(index.row(), COLUMN_ACCOUNT), QtCore.Qt.UserRole):
            combobox.addItem(current_account.currency.name, current_account.currency)
            if not current_account.currency.is_local:
                rmb = System.currency('人民币')
                combobox.addItem(rmb.name, rmb)
        #
        return combobox

    def setModelData(self, editor, model, index):
        combobox: QtWidgets.QComboBox = editor
        currency = combobox.currentData(QtCore.Qt.UserRole)
        model.setData(index, currency.name, QtCore.Qt.ItemDataRole.EditRole)
        model.setData(index, currency, QtCore.Qt.ItemDataRole.UserRole)
        model.setData(index, datetime.datetime.now(), QtCore.Qt.ItemDataRole.StatusTipRole)
...
class VoucherEditTableWidget(QtWidgets.QTableWidget):
    """"""
    def __init__(self):
        super().__init__()
        self.setupUI()
    ...
    def setupUI(self):
        ...
        self.currencyAmountItemDelegate = CurrencyAmountItemDelegate(self)
        ...
        self.setItemDelegateForColumn(COLUMN_DEBIT_CURRENCY_AMOUNT, self.currencyAmountItemDelegate)
        self.setItemDelegateForColumn(COLUMN_CREDIT_CURRENCY_AMOUNT, self.currencyAmountItemDelegate)
        self.setItemDelegateForColumn(COLUMN_CURRENCY, self.currencyItemDelegate)
...
class VoucherEditWidget(QtWidgets.QWidget):

    def __init__(self, date_month: datetime.date):
        super().__init__()
        self.setupUI()
        self._readonly = False
        self.setDateMonth(date_month)
        self.dateedit.dateChanged.connect(lambda *args: (self.action_save.setEnabled(True), self.refresh(True)))
        self.table.tb_choose_account.clicked.connect(self.on_tb_chooseAccountClicked)
        self.startTimer(200)

    def setDateMonth(self, date_month: datetime.date):
        ...
        self.vouchers = self.vouchersInThisMonth()
    ...
    def setupUI(self):
        ...
        self.dateedit.setStyleSheet('color: blue;')
        ...
        self.table = VoucherEditTableWidget()
        ...
        self.action_insert_row_last.triggered.connect(self.on_action_insertRowLastTriggered)
        ...
        self.action_remove_current_row.triggered.connect(self.on_action_removeCurrentRowTriggered)
        ...
        self.action_first.triggered.connect(self.on_action_firstTriggered)
        ...
        self.action_last.triggered.connect(self.on_action_lastTriggered)
        ...
        self.action_back.triggered.connect(self.on_action_backTriggered)
        ...
        self.action_forward.triggered.connect(self.on_action_forwardTriggered)
        ...
        self.action_save.triggered.connect(self.on_action_saveTriggered)
        ...
        self.action_void.triggered.connect(self.on_action_voidTriggered)
    ...
    def setCurrentVoucher(self, voucher_number: str):
        if index := next((i for i, v in enumerate(self.vouchers) if v.number == voucher_number), None):
    ...
    def setReadOnly(self, ro: bool):
        self._readonly = ro
        self.dateedit.setEnabled(not ro)
    ...
    def save(self) -> bool:
        ...
        try:
            System.setVoucherDate(
                ...
                qdate_to_date(self.dateedit.date())
            )
            System.updateDebitCreditEntries(
                self.vouchers[self.index_current].number,
                *self.inputVoucherEntries()
            )
            # update local cache
    ...
    def refreshLocalDebitCreditTotal(self):
    ...
    def refresh(self, force=False):
        ...
        def refreshLocalAmount(row):
            ...
            if exchange_rate and debit_currency_amount:
                self.table.item(row, COLUMN_DEBIT_LOCAL_AMOUNT).setText(
                    str(debit_currency_amount * exchange_rate)
                )
                self.table.item(row, COLUMN_DEBIT_LOCAL_AMOUNT).setData(
                    QtCore.Qt.ItemDataRole.UserRole,
                    debit_currency_amount * exchange_rate
                )
            elif exchange_rate and credit_currency_amount:
                self.table.item(row, COLUMN_CREDIT_LOCAL_AMOUNT).setText(
                    str(credit_currency_amount * exchange_rate)
                )
                self.table.item(row, COLUMN_CREDIT_LOCAL_AMOUNT).setData(
                    QtCore.Qt.ItemDataRole.UserRole,
                    credit_currency_amount * exchange_rate
                )
            #
            self.refreshLocalDebitCreditTotal()
        ...
        for row in range(self.table.rowCount()):
            ...
            if isRefreshNeeded(self.table.item(row, COLUMN_ACCOUNT)) or force:
                ...
                if account and account.currency:
                    ...
                    self.table.item(row, COLUMN_EXCHANGE_RATE).setText(
                        str(System.exchangeRate(account.currency.name, qdate_to_date(self.dateedit.date())).rate)
                    )
                    self.table.item(row, COLUMN_EXCHANGE_RATE).setData(
                        QtCore.Qt.ItemDataRole.UserRole,
                        System.exchangeRate(account.currency.name, qdate_to_date(self.dateedit.date())).rate
                    )
            ...
            if isRefreshNeeded(self.table.item(row, COLUMN_CURRENCY)):
                currency = self.table.item(row, COLUMN_CURRENCY).data(QtCore.Qt.UserRole)
                self.table.item(row, COLUMN_EXCHANGE_RATE).setText(
                    str(System.exchangeRate(currency.name, qdate_to_date(self.dateedit.date())).rate)
                )
                self.table.item(row, COLUMN_EXCHANGE_RATE).setData(
                    QtCore.Qt.ItemDataRole.UserRole,
                    System.exchangeRate(currency.name, qdate_to_date(self.dateedit.date())).rate
                )
                setDirty(self.table.item(row, COLUMN_EXCHANGE_RATE))
    ...
    def updateUI(self):
        ...
        if self.index_current != -1:
            ...
            self.dateedit.setDate(voucher.date)
    ...
    def loadVoucherIntoTable(self, voucher: Voucher):
        ...
        for r, entry in enumerate(voucher.debit_entries):
            ...
            self.table.item(r, COLUMN_CURRENCY).setText(entry.currency)
            self.table.item(r, COLUMN_CURRENCY).setData(
                QtCore.Qt.ItemDataRole.UserRole,
                System.currency(entry.currency)
            )
            self.table.item(r, COLUMN_EXCHANGE_RATE).setText(str(FloatWithPrecision(entry.exchange_rate)))
            self.table.item(r, COLUMN_EXCHANGE_RATE).setData(
                QtCore.Qt.ItemDataRole.UserRole,
                entry.exchange_rate
            )
            self.table.item(r, COLUMN_DEBIT_LOCAL_AMOUNT).setText(
                str(entry.exchange_rate * entry.amount))
            self.table.item(r, COLUMN_DEBIT_LOCAL_AMOUNT).setData(
                QtCore.Qt.ItemDataRole.UserRole,
                entry.exchange_rate * entry.amount
            )
        # !for
        ...
        for i, entry in enumerate(voucher.credit_entries):
            ...
            self.table.item(r, COLUMN_CREDIT_CURRENCY_AMOUNT).setText(str(entry.amount))
            ...
            self.table.item(r, COLUMN_EXCHANGE_RATE).setText(str(entry.exchange_rate))
            self.table.item(r, COLUMN_EXCHANGE_RATE).setData(
                QtCore.Qt.ItemDataRole.UserRole,
                entry.exchange_rate
            )
    ...
    def on_action_removeCurrentRowTriggered(self):
        row = self.table.currentRow()
        if row != -1:
            self.table.removeRow(row)

    def on_action_insertRowLastTriggered(self):
        row = self.table.rowCount() - 1
        self.table.insertRow(row)

    def on_action_saveTriggered(self):
        self.save()

    def on_action_voidTriggered(self):
    ...
    def on_action_forwardTriggered(self):
        if not self.save():
            return
    ...
    def on_action_backTriggered(self):
        if self.save():
            self.index_current -= 1
            self.updateUI()

    def on_action_firstTriggered(self):
        if self.save():
            self.index_current = 0
            self.updateUI()

    def on_action_lastTriggered(self):
        if self.save():
            self.index_current = len(self.vouchers) - 1
            self.updateUI()

    def on_tb_chooseAccountClicked(self):
...
class MonthEndCarryForwardWidget(QtWidgets.QWidget):
    ...
    def setDebitEntryAtTableRow(self, row: int, debit_entry: VoucherEntry):
        ...
        currency = System.currency(debit_entry.currency)
        ...
        self.table.item(row, COLUMN_EXCHANGE_RATE).setData(
            QtCore.Qt.UserRole,
            FloatWithPrecision(debit_entry.exchange_rate)
        )
        self.table.item(row, COLUMN_DEBIT_CURRENCY_AMOUNT).setText(
        ...
        self.table.item(row, COLUMN_DEBIT_LOCAL_AMOUNT).setText(
            str(FloatWithPrecision(debit_entry.amount) * debit_entry.exchange_rate)
        )
        self.table.item(row, COLUMN_DEBIT_LOCAL_AMOUNT).setData(
            QtCore.Qt.UserRole,
            FloatWithPrecision(debit_entry.amount) * debit_entry.exchange_rate
        )
    ...
    def setCreditEntryAtTableRow(self, row: int, credit_entry: VoucherEntry):
        ...
        currency = System.currency(credit_entry.currency)
        self.table.item(row, COLUMN_CURRENCY).setText(currency.name)
        self.table.item(row, COLUMN_CURRENCY).setData(
            QtCore.Qt.UserRole,
            currency
        ...
        self.table.item(row, COLUMN_EXCHANGE_RATE).setText(
            str(FloatWithPrecision(credit_entry.exchange_rate))
        )
        self.table.item(row, COLUMN_EXCHANGE_RATE).setData(
            QtCore.Qt.UserRole,
            FloatWithPrecision(credit_entry.exchange_rate)
        )
        self.table.item(row, COLUMN_CREDIT_CURRENCY_AMOUNT).setText(
        ...
        self.table.item(row, COLUMN_CREDIT_LOCAL_AMOUNT).setText(
            str(FloatWithPrecision(credit_entry.amount) * credit_entry.exchange_rate)
        )
        self.table.item(row, COLUMN_CREDIT_LOCAL_AMOUNT).setData(
            QtCore.Qt.UserRole,
            FloatWithPrecision(credit_entry.amount) * credit_entry.exchange_rate
        )
...
class YearEndCarryForwardWidget(MonthEndCarryForwardWidget):
